...
const login = function(URL, Api_Key, callback) {
    ...
    if (this.URL == '' || this.URL == undefined) {
        throw 'You did not provide a URL for Client login()';
    } else if (this.Key == '' || this.Key == undefined) {
        throw 'You did not provide a API Key for Client login()';
    } else {
...
const request = function(x, callback) {
    axios.get(x.URL + '/api/client', {
       ...
       if (response.status == 404) {
        ...
        console.log(new Error('Could not connect! (Make sure you have your URL correctly typed)'));
    });
}
